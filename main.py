...
- POST /check_sms — URLs stripped from the message string, then text classification.
- POST /check_message — HTML stripped where needed, URLs removed from plain text, then classification.
...
# model yields false phishing flags; URLs are stripped from plain text before classification.
...
def _predict_email_text_only(raw_message: str) -> CombinedPredictionResponse:
    """Email-style scan: strip HTML when present, remove URL strings from plain text, classify text only."""
    ...
    print(
        _ascii_preview(
            f"[email_text_only] mode={content_mode} text={'yes' if text_check is not None else 'no'} overall={combined.result}",
            limit=300,
        )
    )
    return combined
...
def check_sms(req: SmsRequest) -> CombinedPredictionResponse:
    # TEXT MODEL ONLY (no URL scanning).
    # Normalize + remove URLs from the input before classification.
    ...
    print(
        _ascii_preview(
            f"[check_sms] text={'yes' if text_check is not None else 'no'} overall={combined.result}",
            limit=300,
        )
    )
    return combined
...
def check_message(req: SmsRequest) -> CombinedPredictionResponse:
    # TEXT MODEL ONLY (no URL scanning).
    # Gmail / email bodies — HTML-aware extraction + URL removal only.
...
def check_message_text_only(req: SmsRequest) -> CombinedPredictionResponse:
    """Same as /check_message — email/HTML body, URLs stripped, text classifier only."""
    return _predict_email_text_only(req.message)
